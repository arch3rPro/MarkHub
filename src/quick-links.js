...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // 全局变量跟踪当前菜单和清理函数
  let currentQuickLinkMenu = null;
  let currentQuickLinkCloseHandler = null;
  ...
  function showContextMenu(e, site) {
    ...
    console.log('Showing context menu for:', site);

    // 清理之前的菜单和事件监听器
    if (currentQuickLinkMenu) {
      console.log('Removing existing context menu');
      currentQuickLinkMenu.remove();
      currentQuickLinkMenu = null;
    }
    if (currentQuickLinkCloseHandler) {
      document.removeEventListener('click', currentQuickLinkCloseHandler);
      currentQuickLinkCloseHandler = null;
    }

    const contextMenu = document.createElement('div');
    currentQuickLinkMenu = contextMenu;
    ...
    // 保存引用以便后续清理
    currentQuickLinkCloseHandler = closeMenu;
  ...
  function editQuickLink(site) {
    const editDialog = document.getElementById('edit-quicklink-dialog');
    const editNameInput = document.getElementById('edit-quicklink-name');
    const editUrlInput = document.getElementById('edit-quicklink-url');
    const editForm = document.getElementById('edit-quicklink-form');
    const cancelButton = document.querySelector('.cancel-quicklink-button');
    const closeButton = document.querySelector('.close-quicklink-button');
