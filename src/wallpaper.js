...
class WallpaperManager {
    constructor() {
        ...
        this.wallpaperBackground = document.getElementById('wallpaper-background');
        ...
        // 初始化必应壁纸
        this.bingWallpapers = [];
        this.currentBingRegion = localStorage.getItem('bingRegion') || 'zh-CN';
        this.initBingWallpapers();
        this.initBingRegionSelect();
    }

    // 初始化必应壁纸地区选择
    initBingRegionSelect() {
        const regionSelect = document.getElementById('bing-region-select');
        if (regionSelect) {
            regionSelect.value = this.currentBingRegion;
            regionSelect.addEventListener('change', (e) => {
                this.currentBingRegion = e.target.value;
                localStorage.setItem('bingRegion', this.currentBingRegion);
                this.initBingWallpapers();
            });
        }
    }
    ...
    async initializeWallpaper() {
        ...
        if (useDefaultBackground === 'true') {
            ...
            if (this.wallpaperBackground) {
                this.wallpaperBackground.style.backgroundImage = 'none';
            }
            return;
        }
    ...
    applyWallpaper(url) {
        ...
        requestAnimationFrame(() => {
            ...
            // 使用 wallpaper-background 元素
            if (this.wallpaperBackground) {
                this.wallpaperBackground.style.backgroundImage = `url("${url}")`;
            } else {
                // 回退逻辑，以防元素不存在
    ...
    async fetchBingWallpapers(count = 4) {
        try {
            const region = this.currentBingRegion || 'zh-CN';
            
            // 通过 background.js 获取数据以避免 CORS 问题
            const response = await chrome.runtime.sendMessage({
                action: 'fetchBingWallpaper',
                region: region,
                count: count
            });
            
            if (!response || !response.success) {
                // 静默失败，返回空数组，UI会显示友好提示
                return [];
            }
    ...
    renderBingWallpapers() {
        ...
        if (!this.bingWallpapers || this.bingWallpapers.length === 0) {
            ...
            errorMessage.className = 'bing-wallpapers-error';
            errorMessage.innerHTML = `
                <div class="error-icon">
                    <img src="../images/503.svg" alt="Error" />
                </div>
                <div class="error-text">
                    <div class="error-title">${chrome.i18n.getMessage('bingWallpapersLoadError') || '必应壁纸暂时无法加载'}</div>
                    <div class="error-description">${chrome.i18n.getMessage('bingWallpapersLoadErrorDesc') || '可能是网络连接问题，请稍后重试或选择其他壁纸'}</div>
                </div>
            `;
