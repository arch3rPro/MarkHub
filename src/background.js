...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  // 处理 Bing 壁纸获取请求
  if (request.action === 'fetchBingWallpaper') {
    const { region, count } = request;
    
    // 根据用户选择的地区使用对应的域名
    const domain = region === 'zh-CN' ? 'cn.bing.com' : 'www.bing.com';
    const mkt = region;
    
    // 异步函数获取壁纸
    (async () => {
      try {
        const url = `https://${domain}/HPImageArchive.aspx?format=js&idx=0&n=${count}&mkt=${mkt}&uhd=1&uhdwidth=3840&uhdheight=2160`;
        ...
        if (!data?.images || !Array.isArray(data.images)) {
          sendResponse({ success: false, wallpapers: [] });
          return;
        }
        
        // 处理壁纸数据
        const wallpapers = data.images.map(({ url, title, copyright, startdate }) => ({
        ...
        chrome.storage.local.set({
          ...
          if (sender.tab && sender.tab.id) {
            chrome.tabs.sendMessage(sender.tab.id, {
              ...
              if (chrome.runtime.lastError) {
                // 静默处理错误，这在某些情况下是预期的
                console.log('Message not delivered (content script may not be ready):', chrome.runtime.lastError.message);
              }
            });
          }
                ...
                for (let i = 0; i < 3; i++) {
                  setTimeout(() => {
                    chrome.tabs.sendMessage(tabs[0].id, {
                      ...
                      if (chrome.runtime.lastError) {
                        console.log(`Message not delivered (attempt ${i+1}):`, chrome.runtime.lastError.message);
                      }
                    });
                  }, i * 1000); // 分散发送时间
                }
