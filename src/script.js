...
// 书签悬浮卡片管理器
class BookmarkHoverCard {
  constructor() {
    this.card = null;
    this.showTimeout = null;
    this.hideTimeout = null;
    this.currentBookmark = null;
    this.SHOW_DELAY = 500; // 悬停500ms后显示
    this.HIDE_DELAY = 100; // 移开100ms后隐藏
  }

  init() {
    // 创建悬浮卡片元素
    this.card = document.createElement('div');
    this.card.className = 'bookmark-hover-card';
    document.body.appendChild(this.card);
  }

  show(bookmark, targetElement) {
    // 清除之前的隐藏定时器
    if (this.hideTimeout) {
    ...
    // 如果是同一个书签，不需要重新显示
    if (this.currentBookmark === bookmark.id && this.card.classList.contains('visible')) {
      return;
    }
  ...
  updateContent(bookmark) {
    const faviconUrl = `chrome-extension://${chrome.runtime.id}/_favicon/?pageUrl=${encodeURIComponent(bookmark.url)}&size=32`;
    
    this.card.innerHTML = `
      <div class="bookmark-hover-card-header">
        <div class="bookmark-hover-card-favicon">
          <img src="${faviconUrl}" alt="">
        </div>
        <div class="bookmark-hover-card-title">${this.escapeHtml(bookmark.title)}</div>
      </div>
      <div class="bookmark-hover-card-url">${this.escapeHtml(bookmark.url)}</div>
    `;
  }
...
function openEditDialog(bookmark) {
  ...
  // 从 Chrome API 获取最新的书签数据，确保显示的是最新信息
  chrome.bookmarks.get(bookmarkId, function(results) {
    if (!results || results.length === 0) {
      console.error('无法获取书签信息:', bookmarkId);
      return;
    }
    
    const latestBookmark = results[0];
    const bookmarkTitle = latestBookmark.title;
    const bookmarkUrl = latestBookmark.url;
    const currentParentId = latestBookmark.parentId;

    console.log('编辑书签 - ID:', bookmarkId, '最新标题:', bookmarkTitle, '父文件夹ID:', currentParentId);

    document.getElementById('edit-bookmark-name').value = bookmarkTitle;
    document.getElementById('edit-bookmark-url').value = bookmarkUrl;

    const editDialog = document.getElementById('edit-bookmark-dialog');
    const folderSelector = document.getElementById('edit-bookmark-folder');
    
    // 加载文件夹树
    loadBookmarkFolders(folderSelector, currentParentId);
    ...
    function handleOutsideClick(event) {
      if (event.target === editDialog) {
        ...
        editDialog.removeEventListener('click', handleOutsideClick);
      }
    }
    editDialog.addEventListener('click', handleOutsideClick);

    // 设置提交事件
    document.getElementById('edit-bookmark-form').onsubmit = function (event) {
      event.preventDefault();
      const newTitle = document.getElementById('edit-bookmark-name').value;
      const newUrl = document.getElementById('edit-bookmark-url').value;
      const newParentId = folderSelector.getSelectedFolderId();
      ...
      chrome.bookmarks.update(bookmarkId, { title: newTitle, url: newUrl }, function () {
        ...
        if (newParentId && newParentId !== currentParentId) {
          chrome.bookmarks.move(bookmarkId, { parentId: newParentId }, function() {
            ...
            if (currentFolderId) {
              updateBookmarksDisplay(currentFolderId);
            }
          });
        } else {
          editDialog.style.display = 'none';
    ...
    // 添加取消按钮的事件监听
    document.querySelector('.cancel-bookmark-button').onclick = function () {
      editDialog.style.display = 'none';
      editDialog.removeEventListener('click', handleOutsideClick);
    };

    // 添加关闭按钮的事件监听
    document.querySelector('.close-bookmark-button').onclick = function () {
      editDialog.style.display = 'none';
      editDialog.removeEventListener('click', handleOutsideClick);
    };
  });
...
function updateSpecificBookmarkCard(bookmarkId, newTitle, newUrl) {
  console.log('更新书签卡片:', bookmarkId, newTitle, newUrl);
  const bookmarkCard = document.querySelector(`.bookmark-card[data-id="${bookmarkId}"]`);
  if (bookmarkCard) {
    console.log('找到书签卡片，正在更新...');
    bookmarkCard.href = newUrl;
    
    const titleElement = bookmarkCard.querySelector('.card-title');
    if (titleElement) {
      titleElement.textContent = newTitle;
      console.log('标题已更新为:', newTitle);
    } else {
      console.error('未找到 .card-title 元素');
    }
    ...
    // 如果当前书签是右键菜单中缓存的书签，也更新缓存
    if (typeof currentBookmark !== 'undefined' && currentBookmark && currentBookmark.id === bookmarkId) {
      currentBookmark.title = newTitle;
      currentBookmark.url = newUrl;
      console.log('已更新右键菜单缓存的书签信息');
    }
  } else {
    console.error('未找到书签卡片，ID:', bookmarkId);
  }
}
...
// 加载书签文件夹列表（树形选择器）
function loadBookmarkFolders(containerElement, currentParentId) {
  console.log('loadBookmarkFolders 被调用 - currentParentId:', currentParentId); // 调试日志
  
  const listElement = containerElement.querySelector('.folder-tree-list');
  const displayElement = containerElement.querySelector('.selected-folder-name');
  const dropdownElement = containerElement.querySelector('.folder-tree-dropdown');
  
  console.log('displayElement:', displayElement); // 调试日志
  ...
  // 移除所有选中状态
  containerElement.querySelectorAll('.folder-tree-item.selected').forEach(item => {
    item.classList.remove('selected');
  });
  
  // 首先获取当前文件夹的名称并立即显示
  if (currentParentId) {
    console.log('正在获取文件夹信息 - ID:', currentParentId); // 调试日志
    chrome.bookmarks.get(currentParentId, function(results) {
      console.log('chrome.bookmarks.get 返回:', results); // 调试日志
      if (results && results[0]) {
        selectedFolderName = results[0].title;
        displayElement.textContent = selectedFolderName;
        console.log('设置文件夹名称:', selectedFolderName); // 调试日志
      } else {
        console.log('未找到文件夹信息'); // 调试日志
      }
    });
  } else {
    console.log('currentParentId 为空'); // 调试日志
    displayElement.textContent = chrome.i18n.getMessage('selectFolder') || '请选择文件夹';
  }
  
  // 获取书签树
  chrome.bookmarks.getTree(function(bookmarkTreeNodes) {
    
    function createFolderItem(node, level = 0, parentContainer = null) {
      if (!node.url && node.id !== '0') {
        const item = document.createElement('div');
        item.className = 'folder-tree-item';
        item.dataset.folderId = node.id;
        item.dataset.folderName = node.title;
        ...
        if (hasChildren) {
          ...
          toggle.innerHTML = '<span class="material-icons">chevron_right</span>';
        ...
        if (node.id === currentParentId) {
          ...
          selectedFolderName = node.title;
        }
        
        // 点击选择
        item.onclick = (e) => {
          if (!e.target.closest('.folder-toggle')) {
            selectFolder(node.id, node.title);
          }
        };
        
        const targetContainer = parentContainer || listElement;
        ...
        if (hasChildren) {
          ...
          node.children.forEach(child => {
            createFolderItem(child, level + 1, childContainer);
          });
    ...
    function selectFolder(folderId, folderName) {
      // 移除之前的选中状态
      listElement.querySelectorAll('.folder-tree-item').forEach(item => {
        item.classList.remove('selected');
      });
      
      // 设置新的选中状态
      const selectedItem = listElement.querySelector(`[data-folder-id="${folderId}"]`);
      if (selectedItem) {
    ...
    // 构建文件夹树
    bookmarkTreeNodes[0].children.forEach(node => {
      createFolderItem(node, 0);
    });
  });
  
  // 移除旧的事件监听器（如果存在）
  const displayArea = containerElement.querySelector('.folder-selected-display');
  const oldClickHandler = displayArea._clickHandler;
  if (oldClickHandler) {
    displayArea.removeEventListener('click', oldClickHandler);
  }
  
  // 点击显示区域切换下拉框
  const newClickHandler = (e) => {
    e.stopPropagation();
    const isOpen = containerElement.classList.contains('open');
  ...
  displayArea._clickHandler = newClickHandler;
  displayArea.addEventListener('click', newClickHandler);
  
  // 移除旧的外部点击监听器
  if (containerElement._closeDropdownHandler) {
    document.removeEventListener('click', containerElement._closeDropdownHandler);
  }
  ...
  containerElement._closeDropdownHandler = closeDropdownHandler;
  document.addEventListener('click', closeDropdownHandler);
  
  // 返回获取选中文件夹ID的方法
  containerElement.getSelectedFolderId = () => {
    const selectedItem = listElement.querySelector('.folder-tree-item.selected');
    return selectedItem ? selectedItem.dataset.folderId : currentParentId;
  };
}
...
function updateThemeIcon(isDark) {
  ...
  if (savedTheme === 'auto') {
    ...
    iconHtml = isDark ? ICONS.dark_mode : ICONS.light_mode;
  }
...
document.addEventListener('DOMContentLoaded', function () {
  // 初始化书签悬浮卡片
  bookmarkHoverCard = new BookmarkHoverCard();
  bookmarkHoverCard.init();
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  chrome.storage.sync.get(['bookmarkContainerWidth'], (result) => {
    ...
    const maxWidthValue = savedWidth > 75 ? 'none' : '';
  ...
  chrome.storage.sync.get(
    ...
    (result) => {
      ...
      // 应用显示书签数量设置
      if (result.showBookmarkCount !== false) {
          document.body.dataset.showBookmarkCount = "true";
      }

      // 应用显示目录树图标设置
      if (result.showFolderIcons !== false) {
...
async function updateBookmarkCards() {
  ...
  chrome.bookmarks.getChildren(parentId, async function (bookmarks) {
    await displayBookmarks({ id: parentId, children: bookmarks });
...
function updateBookmarksDisplay(parentId, movedItemId, newIndex) {
  return new Promise(async (resolve, reject) => {
    ...
    if (cached && !movedItemId) {
      ...
      await displayBookmarks({ id: parentId, children: cached.bookmarks });
      resolve();
      return;
    }

    // 如果没有缓存或是移动操作，从 Chrome API 获取数据
    chrome.bookmarks.getChildren(parentId, async (bookmarks) => {
      if (chrome.runtime.lastError) {
        reject(chrome.runtime.lastError);
        return;
      }
...
async function displayBookmarks(bookmark) {
  const bookmarksList = document.getElementById('bookmarks-list');
  const bookmarksContainer = document.querySelector('.bookmarks-container');
  if (!bookmarksList) {
    return;
  ...
  // 区域1: 当前目录下的子目录卡片
  const folders = itemsToDisplay.filter(child => !child.url);
  if (folders.length > 0) {
    folders.forEach((folder) => {
      const folderCard = createFolderCard(folder, folder.index);
      fragment.appendChild(folderCard);
    });
  }
  
  // 区域2: 当前目录下的书签（非文件夹）- 单独另起一行
  const currentBookmarks = itemsToDisplay.filter(child => child.url);
  if (currentBookmarks.length > 0) {
    // 创建一个分隔区域
    const currentBookmarksSection = document.createElement('div');
    currentBookmarksSection.className = 'current-bookmarks-section';
    currentBookmarksSection.style.width = '100%';
    currentBookmarksSection.style.display = 'flex';
    currentBookmarksSection.style.flexWrap = 'wrap';
    currentBookmarksSection.style.gap = '1rem';
    currentBookmarksSection.style.marginTop = folders.length > 0 ? '1rem' : '0';
    
    currentBookmarks.forEach((child) => {
      const card = createBookmarkCard(child, child.index);
      currentBookmarksSection.appendChild(card);
    });
    
    fragment.appendChild(currentBookmarksSection);
  }
  ...
  for (const folder of folders) {
    try {
      // 获取该子目录的内容（只获取一级子项）
      const subChildren = await new Promise((resolve, reject) => {
      ...
      // 只获取书签（不要子文件夹）
      const subBookmarks = subChildren
      ...
      if (subBookmarks.length > 0) {
        ...
        folderSection.className = 'bookmark-folder-section';
        ...
        folderBookmarksContainer.className = 'folder-bookmarks-container';
        
        // 添加该文件夹的书签
        subBookmarks.forEach((subBookmark) => {
          const card = createBookmarkCard(subBookmark, subBookmark.index);
          folderBookmarksContainer.appendChild(card);
        });
        
        folderSection.appendChild(folderBookmarksContainer);
        fragment.appendChild(folderSection);
      }
    } catch (error) {
      console.error('Error loading subfolder bookmarks:', error);
    }
  }
...
function createBookmarkCard(bookmark, index) {
  ...
  card.draggable = true;  // 启用原生拖放
  
  // 应用保存的书签宽度设置
  const savedWidth = localStorage.getItem('bookmarkWidth') || '190';
  ...
  card.addEventListener('mouseenter', function() {
    ...
    // 显示悬浮卡片
    if (bookmarkHoverCard) {
      bookmarkHoverCard.show(bookmark, this);
    }
  });
...
function showContextMenu(event, item, type = 'bookmark') {
  ...
  currentBookmark = {
    ...
    parentId: item.parentId,  // 保留 parentId 信息
...
function createFolderCard(folder, index) {
  ...
  card.dataset.index = index.toString();
  card.dataset.folderDropTarget = 'true';  // 标记为文件夹拖放目标
  ...
  // 添加目录下的标签数量显示（包括所有子目录）
  getFolderBookmarkCount(folder.id).then(count => {
    const badge = document.createElement('span');
    badge.className = 'folder-count-badge';
    badge.textContent = count;
    
    // 检查是否应该显示计数
    chrome.storage.sync.get('showBookmarkCount', (result) => {
      if (result.showBookmarkCount === false) {
  ...
  card.addEventListener('dragenter', function(e) {
    // 检查是否是书签被拖动
    const draggedElement = document.querySelector('.sortable-drag') || document.querySelector('.sortable-ghost') || document.querySelector('.sortable-chosen');
    console.log('Dragenter on folder card:', {
      folderId: folder.id,
      folderTitle: folder.title,
      hasDraggedElement: !!draggedElement,
      draggedElementClass: draggedElement?.className
    });
    
    if (draggedElement && draggedElement.classList.contains('bookmark-card')) {
      e.preventDefault();
      e.stopPropagation();
      
      // 添加高亮效果
      this.style.backgroundColor = 'rgba(66, 133, 244, 0.2)';
      this.style.border = '3px solid #4285f4';
      this.style.transform = 'scale(1.08)';
      this.style.zIndex = '1000';
      
      console.log('Bookmark entering folder card:', folder.id);
    }
  });
  ...
  card.addEventListener('dragleave', function(e) {
    ...
    if (!this.contains(e.relatedTarget)) {
      ...
      console.log('Bookmark left folder card:', folder.id);
    }
  });
  ...
  card.addEventListener('drop', function(e) {
    ...
    console.log('Drop event on folder card:', folder.id);
    ...
    const draggedElement = document.querySelector('.sortable-drag') || document.querySelector('.sortable-ghost') || document.querySelector('.sortable-chosen');
    if (draggedElement && draggedElement.classList.contains('bookmark-card')) {
      const bookmarkId = draggedElement.dataset.id;
      const targetFolderId = folder.id;
      
      console.log('Bookmark dropped on folder card:', {
        bookmarkId,
        targetFolderId,
        folderTitle: folder.title
      });
      ...
      // 标记已经通过原生事件处理了拖放
      // 这样 Sortable 的 onEnd 就不会重复处理
      if (!window.setupSortableContext) {
        window.setupSortableContext = {};
      }
      window.setupSortableContext.droppedOnFolderCard = true;
      window.setupSortableContext.droppedBookmarkId = bookmarkId;
      ...
      // 立即移除原位置的书签卡片（包括 ghost 和原始元素）
      const allMatchingCards = document.querySelectorAll(`[data-id="${bookmarkId}"]`);
      allMatchingCards.forEach(card => {
        console.log('Removing card element:', card.className, card.dataset.id);
        card.remove();
      });
      
      // 多次尝试清除残影元素，确保清除干净
      const removeGhosts = () => {
        const ghostElements = document.querySelectorAll('.sortable-ghost, .sortable-drag, .sortable-chosen, .sortable-fallback');
        let removed = 0;
        ghostElements.forEach(el => {
          console.log('Removing ghost element:', el.className);
          el.remove();
          removed++;
        });
        console.log('Removed ghost elements:', removed);
        return removed;
      };
      
      // 立即执行
      removeGhosts();
      setTimeout(removeGhosts, 10);
      ...
      chrome.bookmarks.move(bookmarkId, {
        ...
        if (chrome.runtime.lastError) {
          console.error('Error moving bookmark to folder:', chrome.runtime.lastError.message);
        } else {
          console.log('Bookmark moved to folder successfully:', result);
        }
        // 刷新页面
        if (currentParentId) {
  ...
  card.addEventListener('click', function() {
    ...
    selectSidebarFolder(folder.id);
    // 保存最后访问的文件夹
    chrome.storage.local.set({ 
      lastViewedFolder: folder.id,
      lastViewedTime: Date.now()
    });
  });
...
function setupSortable() {
  ...
  if (bookmarksList) {
    ...
    let droppedOnFolderCard = false;  // 标记是否已经通过原生事件处理了拖放
    let droppedBookmarkId = null;
    
    // 创建全局上下文，供文件夹卡片的 drop 事件访问
    window.setupSortableContext = {
      droppedOnFolderCard: false,
      droppedBookmarkId: null
    };
    
    // 为主书签列表设置 Sortable（处理文件夹卡片和接收书签）
    const mainListSortable = new Sortable(bookmarksList, {
      animation: 150,
      group: {
        name: 'bookmarks-main',
        pull: false,
        put: false  // 禁止接收任何元素，书签只能拖到文件夹卡片上
      },
      draggable: '.bookmark-folder',  // 只有文件夹可以被拖动排序
      filter: '.bookmark-card',  // 过滤掉书签卡片，不允许在主列表拖动
      
      onEnd: function (evt) {
        // 检查是否已经通过原生 drop 事件处理了
        if (window.setupSortableContext && window.setupSortableContext.droppedOnFolderCard) {
          const droppedId = window.setupSortableContext.droppedBookmarkId;
          const currentId = evt.item.dataset.id;
          
          console.log('Sortable onEnd: Already handled by native drop event', {
            droppedId,
            currentId,
            match: droppedId === currentId
          });
          
          // 重置标记
          window.setupSortableContext.droppedOnFolderCard = false;
          window.setupSortableContext.droppedBookmarkId = null;
          
          // 如果是同一个书签，跳过处理
          if (droppedId === currentId) {
            console.log('Skipping Sortable onEnd for bookmark', currentId);
            ...
            // 清除所有文件夹的高亮
            const folders = bookmarksList.querySelectorAll('.bookmark-folder');
            folders.forEach(folder => {
    ...
    // 为当前目录的书签容器设置 Sortable
    const currentBookmarksSection = bookmarksList.querySelector('.current-bookmarks-section');
    if (currentBookmarksSection) {
      const currentSectionSortable = new Sortable(currentBookmarksSection, {
        animation: 150,
        group: {
          name: 'bookmarks',
          pull: 'bookmarks',  // 只允许拖到其他 bookmarks 组
          put: true
        },
        ...
        onEnd: function (evt) {
          // 检查是否已经通过原生 drop 事件处理了
          const itemId = evt.item.dataset.id;
          if (window.setupSortableContext?.droppedOnFolderCard && 
              window.setupSortableContext.droppedBookmarkId === itemId) {
            console.log('Skipping current section onEnd - already handled by native drop:', itemId);
          ...
          console.log('Current section onEnd:', {
            itemId,
            isCrossContainer,
            fromClassName: evt.from.className,
            toClassName: evt.to.className,
            currentParentId
          });
          
          if (isCrossContainer) {
            // 判断拖动方向
            const isFromFolderToCurrentSection = evt.from.classList.contains('folder-bookmarks-container');
            const isFromCurrentSectionToFolder = evt.to.classList.contains('folder-bookmarks-container');
            
            console.log('Cross-container direction:', {
              isFromFolderToCurrentSection,
              isFromCurrentSectionToFolder
            });
            
            if (isFromFolderToCurrentSection) {
              // 从区域3（文件夹容器）拖到区域2（当前目录）
              const sourceFolderSection = evt.from.closest('.bookmark-folder-section');
              const sourceFolderId = sourceFolderSection?.dataset.folderId;
              
              console.log('Cross-container move: folder to current section:', {
                itemId,
                sourceFolderId,
              ...
              chrome.bookmarks.move(itemId, {
                ...
                if (chrome.runtime.lastError) {
                  console.error('Error moving bookmark to current section:', chrome.runtime.lastError.message);
                } else {
                  console.log('Bookmark moved to current section successfully:', result);
                }
                // 跨容器移动必须立即刷新以同步所有区域的 DOM
                // 清除缓存以确保获取最新数据
                bookmarksCache.clear();
                updateBookmarksDisplay(currentParentId);
              });
            } else if (isFromCurrentSectionToFolder) {
              // 从区域2拖到区域3（文件夹容器）
              const targetFolderSection = evt.to.closest('.bookmark-folder-section');
              const targetFolderId = targetFolderSection?.dataset.folderId;
              
              console.log('Cross-container move: current section to folder:', {
                itemId,
              ...
              if (targetFolderId) {
                ...
                console.error('Target folder ID not found');
              }
            }
          } else {
            // 如果是在同一容器内移动（区域2内部排序）
            moveBookmark(itemId, currentParentId, evt.newIndex)
              .catch(error => console.error('Error moving bookmark:', error));
          }
        }
      });
      sortableInstances.push(currentSectionSortable);
    }
    
    // 为每个文件夹分组的书签容器设置 Sortable
    const folderBookmarksContainers = bookmarksList.querySelectorAll('.folder-bookmarks-container');
    console.log('Setting up Sortable for folder containers, found:', folderBookmarksContainers.length);
    
    folderBookmarksContainers.forEach((container, index) => {
      const folderSection = container.closest('.bookmark-folder-section');
      const folderId = folderSection?.dataset.folderId;
      
      console.log(`Folder container ${index}:`, {
        folderId,
        hasSection: !!folderSection,
        containerClass: container.className
      });
      
      if (!folderId) {
        console.warn('Skipping container without folderId');
        return;
      }
      
      const folderContainerSortable = new Sortable(container, {
        animation: 150,
        group: {
          name: 'bookmarks',
          pull: 'bookmarks',  // 只允许拖到其他 bookmarks 组
        ...
        onEnd: function (evt) {
          ...
          if (window.setupSortableContext?.droppedOnFolderCard && 
              window.setupSortableContext.droppedBookmarkId === itemId) {
            console.log('Skipping Sortable onEnd - already handled by native drop:', itemId);
          ...
          // 从 DOM 重新获取目标文件夹 ID，而不是依赖闭包
          const targetContainer = evt.to;
          const targetFolderSection = targetContainer.closest('.bookmark-folder-section');
          
          // 检查是否拖到了区域2（当前目录区域）
          const isTargetCurrentSection = targetContainer.classList.contains('current-bookmarks-section');
          
          // 如果拖到了区域2，目标父ID是当前目录ID
          const actualTargetFolderId = isTargetCurrentSection ? currentParentId : (targetFolderSection?.dataset.folderId || folderId);
          
          // 获取源文件夹 ID
          const sourceContainer = evt.from;
          const sourceFolderSection = sourceContainer.closest('.bookmark-folder-section');
          const sourceFolderId = sourceFolderSection?.dataset.folderId;
          
          const isCrossContainer = evt.from !== evt.to;
          const isCrossFolder = sourceFolderId !== actualTargetFolderId;
          
          console.log('Folder container onEnd:', {
            itemId,
            closureFolderId: folderId,
            sourceFolderId: sourceFolderId,
            actualTargetFolderId: actualTargetFolderId,
            isTargetCurrentSection: isTargetCurrentSection,
            fromContainer: evt.from.className,
            toContainer: evt.to.className,
            isCrossContainer: isCrossContainer,
            isCrossFolder: isCrossFolder,
            newIndex: evt.newIndex
          });
          
          // 先获取书签的当前信息，确定它的实际父文件夹
          chrome.bookmarks.get(itemId, (bookmarks) => {
            if (chrome.runtime.lastError || !bookmarks || bookmarks.length === 0) {
              console.error('Error getting bookmark info:', chrome.runtime.lastError?.message);
              return;
            }
            
            const bookmark = bookmarks[0];
            const currentBookmarkParentId = bookmark.parentId;
            
            console.log('Bookmark current state:', {
              itemId,
              currentParentId: currentBookmarkParentId,
              targetFolderId: actualTargetFolderId,
              needsMove: currentBookmarkParentId !== actualTargetFolderId
            });
            
            // 如果书签已经在目标文件夹中，只是重新排序
            if (currentBookmarkParentId === actualTargetFolderId) {
              console.log('Same folder reorder: Moving bookmark', itemId);
              
              // 使用书签的当前 index 和目标 index
              const currentIndex = bookmark.index;
              let targetIndex = evt.newIndex;
              
              // Chrome Bookmarks API 在同一父文件夹内移动时，不会先移除再插入
              // 而是直接修改 index 属性，所以如果从前往后移，需要 -1
              // 如果从后往前移，不需要调整
              if (currentIndex < targetIndex) {
                // 从前往后移，目标索引需要 -1（因为当前元素还在那里）
                targetIndex = targetIndex - 1;
              }
              
              console.log('Reorder in same folder:', {
                itemId,
                folderId: actualTargetFolderId,
                currentIndex: currentIndex,
                sortableNewIndex: evt.newIndex,
                sortableOldIndex: evt.oldIndex,
                adjustedTargetIndex: targetIndex
              });
              
              // 只有当位置真的改变了才执行移动
              if (currentIndex !== targetIndex) {
                // 同文件夹排序：直接调用 Chrome API，不需要更新 UI（Sortable 已经更新了）
                chrome.bookmarks.move(itemId, {
                  parentId: actualTargetFolderId,
                  index: targetIndex
                }, (result) => {
                  if (chrome.runtime.lastError) {
                    console.error('Error reordering bookmark:', chrome.runtime.lastError.message);
                    // 出错时刷新页面恢复状态
                    updateBookmarksDisplay(currentParentId);
                  } else {
                    console.log('Bookmark reordered successfully in folder', actualTargetFolderId, 'from index', currentIndex, 'to', targetIndex);
                  }
                });
              } else {
                console.log('Bookmark position unchanged, skip move');
              }
            } else {
              // 书签需要移动到新文件夹
              console.log('Cross-folder move: Moving bookmark', itemId, 'from', currentBookmarkParentId, 'to', actualTargetFolderId);
              ...
              chrome.bookmarks.move(itemId, {
                ...
                if (chrome.runtime.lastError) {
                  ...
                  console.log('Bookmark moved to folder:', result);
                  // 成功后立即刷新，确保 UI 同步
                  // 清除缓存以确保获取最新数据
                  bookmarksCache.clear();
                  updateBookmarksDisplay(currentParentId);
      ...
      sortableInstances.push(folderContainerSortable);
    });
  }
...
function setupFolderDropTargets() {
  // 这个函数现在不需要了，所有拖放逻辑都在 setupSortable 中处理
  console.log('Folder drop targets setup - handled by setupSortable');
}

// 为书签卡片添加拖放数据
// 注意：Sortable.js 已经处理了拖放功能，这个函数不再需要
function enhanceBookmarkCardDragging() {
  // 不需要手动设置 draggable 和 dragstart 事件
  // Sortable.js 会自动处理这些
  console.log('Bookmark card dragging is handled by Sortable.js');
}
...
function moveBookmark(itemId, newParentId, newIndex) {
  return new Promise((resolve, reject) => {
    ...
    if (newParentId) {
      destination.parentId = newParentId;
    }
    
    chrome.bookmarks.move(itemId, destination, (result) => {
      if (chrome.runtime.lastError) {
        console.error('Error moving bookmark:', chrome.runtime.lastError.message || chrome.runtime.lastError);
        reject(chrome.runtime.lastError);
      } else {
        console.log(`Bookmark ${itemId} moved to index ${result.index}`);
        const targetParentId = newParentId || result.parentId;
        updateAffectedBookmarks(targetParentId, itemId, result.index)
          .then(() => {
...
function displayBookmarkCategories(bookmarkNodes, level, parentUl, parentId) {
  ...
  bookmarkNodes.forEach(function (bookmark) {
    if (bookmark.children && bookmark.children.length > 0) {
      ...
      li.className = 'flex items-center folder-item';
      li.style.paddingLeft = `${(level * 12) + 4}px`;
      ...
      categoryContainer.className = 'flex items-center cursor-pointer category-btn';
      categoryContainer.style.padding = '4px 4px';
      categoryContainer.style.borderRadius = '4px';
      categoryContainer.style.flex = '1';

      const folderIcon = document.createElement('span');
      folderIcon.className = 'material-icons mr-2 folder-icon';
      folderIcon.innerHTML = ICONS.folder;
      categoryContainer.appendChild(folderIcon);
      ...
      if (hasSubfolders) {
        // 创建独立的折叠按钮，完全在分类容器外
        arrowIcon = document.createElement('button');
        arrowIcon.className = 'fold-button material-icons text-gray-500 hover:text-gray-800 focus:outline-none';
        arrowIcon.innerHTML = ICONS.chevron_right;
        arrowIcon.style.border = 'none';
        arrowIcon.style.padding = '4px 6px';
        arrowIcon.style.cursor = 'pointer';
        arrowIcon.style.marginLeft = 'auto';
        arrowIcon.style.borderRadius = '4px';
        arrowIcon.style.transition = 'background-color 0.2s ease';
      ...
      categoryContainer.addEventListener('click', function (event) {
        ...
        updateFolderName(bookmark.id);
        selectSidebarFolder(bookmark.id);
        // 保存最后访问的文件夹
        chrome.storage.local.set({ 
          lastViewedFolder: bookmark.id,
          lastViewedTime: Date.now()
        });
      });
...
// 添加一个获取文件夹内书签数量的函数（递归计算包括所有子目录）
function getFolderBookmarkCount(folderId) {
  return new Promise((resolve) => {
    let count = 0;

    async function countBookmarks(id) {
      return new Promise((res) => {
        chrome.bookmarks.getChildren(id, async (children) => {
          if (chrome.runtime.lastError) {
            res(0);
            return;
          }
...
function openEditBookmarkFolderDialog(folderElement) {
  ...
  function handleOutsideClick(event) {
    if (event.target === editCategoryDialog) {
      ...
      editCategoryDialog.removeEventListener('click', handleOutsideClick);
    }
  }
  editCategoryDialog.addEventListener('click', handleOutsideClick);
  ...
  function closeDialog() {
    editCategoryDialog.style.display = 'none';
    editCategoryDialog.removeEventListener('click', handleOutsideClick);
    if (cancelButton) cancelButton.removeEventListener('click', closeDialog);
    if (closeButton) closeButton.removeEventListener('click', closeDialog);
  }
...
function syncBookmarkOrder(parentId) {
  const cached = bookmarksCache.get(parentId);
  if (!cached || !cached.bookmarks) return;
  ...
  chrome.bookmarks.getChildren(parentId, (bookmarks) => {
    ...
    if (JSON.stringify(chromeOrder) !== JSON.stringify(cachedOrder)) {
      ...
      // 重新渲染当前页 - 使用 updateBookmarksDisplay
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function setSidebarState(isCollapsed) {
    if (isCollapsed) {
      ...
      toggleSidebarButton.innerHTML = getIconHtml('chevron_right');
      toggleSidebarButton.style.left = '2rem'; // 收起时的位置
    } else {
      sidebarContainer.classList.remove('collapsed');
      toggleSidebarButton.innerHTML = getIconHtml('chevron_left');
  ...
  // 注意：openEditDialog、updateSpecificBookmarkCard 和 updateBookmarkCardColors 函数已在全局范围定义
  // 这里不再重复定义，避免覆盖全局函数
